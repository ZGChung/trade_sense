...
export interface AIExplanationResult {
  text: string;
  source: "gemini-free" | "minimax-fallback" | "user-key" | "static-template" | "cache";
...
class AIService {
  ...
  /**
   * Fetches AI explanation for a quiz result.
   *
   * Flow:
   * 1. Build cache lookup key: stockSymbol::sortedEventIds::correctAnswer::userPrediction
   * 2. Call /api/ai-explain (server-side: checks Supabase cache → compute on miss → store)
   * 3. If API fails completely, fall back to static template
   */
  async explainPredictionResult(
    ...
    try {
      const response = await fetchWithTimeout("/api/ai-explain", {
        method: "POST",
        timeoutMs: 25000,
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new AIProviderError(`API 请求失败: ${truncateText(errorText)}`, response.status);
      }
      ...
      const text = data.text?.trim();
      if (!text) {
        throw new AIProviderError("API 返回为空");
      }

      let source: AIExplanationResult["source"] = "gemini-free";
      let providerLabel = "AI 解释";

      if (data.source === "cache") {
        source = "cache";
        providerLabel = "缓存结果 (跨用户共享)";
      } else if (data.source === "minimax") {
        source = "minimax-fallback";
        providerLabel = `MiniMax ${this.minimaxModel}`;
      } else if (data.source === "gemini") {
        source = "gemini-free";
        providerLabel = "Gemini 2.5 Flash-Lite";
      }

      return { text, source, providerLabel };
    } catch (error) {
      const errMsg = error instanceof Error ? error.message : String(error);
      console.error("[aiService] /api/ai-explain failed:", errMsg);

      // Fallback to static template when API completely unavailable
      return {
